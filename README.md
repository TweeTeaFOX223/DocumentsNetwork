...
| `name` | 49 bytes 以下推奨 | 後段のノード読込で `fileName[50]` に `%s` で読込 |
| `normalText`（1行） | 14999 bytes 以下推奨 | 後段のノード読込で `context[15000]` に `%14999s` で読込 |
| `wakachiText` の各トークン | 19 bytes 以下推奨 | 後段のノード読込で `keyword[20]` に `%s` で読込 |
